...
- **47Ω resistor** (1/4W, through-hole — see Resistor Selection Guide below)
...
Summary:
  ...
  ZO        → ESP32 GPIO5 (direct) AND ESP32 GPIO6 (through 47Ω resistor)
...
**Start with 47Ω** (yellow-violet-black-gold) and work from there:

| Resistor | Notes |
|----------|-------|
| **47Ω** | ✅ Start here — most reliable for commands AND status |
| 100Ω | Try if 47Ω causes echo/init issues |
| 220Ω | Try if lower values cause status corruption |
| 330Ω | Try if 220Ω still causes problems |
| 470Ω–1KΩ | Worked on some setups but commands may not go through on others |

**If commands don't work:** try a lower value. The resistor just needs to be enough to prevent the echo from corrupting the Tuya parser — even a small value helps.

**If status is garbled or Tuya init keeps failing:** try a higher value. Work up: 47Ω → 100Ω → 220Ω.
...
2. Try a **different resistor value**. Start with 47Ω, then try 100Ω, 220Ω, 330Ω. The optimal value varies between setups.
...
6. A series resistor on the TX→ZO path was the final breakthrough for half-duplex — **47Ω** proved most reliable, though the optimal value may vary between units (tested range: 47Ω–1KΩ)
...
- The resistor value is sensitive to the specific setup — 47Ω worked most reliably, but values from 47Ω to 1KΩ have all worked in different configurations. The resistor just needs to be enough to prevent the ESP32's TX echo from corrupting the Tuya parser on the shared ZO line
